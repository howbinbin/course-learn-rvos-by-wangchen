...
/*
 * _alloc_start point to the actual address of heap pool
 * _alloc_end points to the actual address of heap
 * _num_pages holds the actualamx number of pages we can allocate.
 * */
 ...
 * Page Descriptor
 * flags:
 * - bits 0: flag if this is taken(allocated)
 * - bits 2: flag if this page is the last of mamory block allocated
 * */
...
void page_init(void)
{
  ...
  /*
   * We reserved some Page to hold Page structures.
   * The number of reserved pages depends on the LENGTH_RAM.
   * For simplicity, the space we reserve here is just on approximation,
  ...
  /*
   * We use HEAP_START, not _heap_start_aligned as begin address for allocating struct Page, 
   * because we heve no requirement of alignment for position struct Page; 
   * */
 ...
 *Allocate a memory block which is composed of contiguous physical pages
 - npages: the number of PAGE_SIZE pagse to allocate
 */

void *page_alloc(int npages)
{
  /*Note we are searching the pagse descriptor bitmaps. */\
  int found = 0;
  ...
  for(int i = 0; i<=(_num_pages - npages); i++)
  {
    if(_is_free(page_i))
    {
      found = 1;
      /*
       * meet a free pages, continue to check if following
       ...
       * address of the first pagse of thi memory block.
       * */
      if(found)
      {
...
void page_free(void *p)
{
  ...
  /*loop and clear all the page descriptor of the memory block*/
  while(!_is_free(page))
  {
    if(_is_last(page))
    {
